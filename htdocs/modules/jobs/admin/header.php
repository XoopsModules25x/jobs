 ...
 * @version         $Id: header.php 2022 2008-08-31 02:07:17Z phppp $
 */
...
function loadModuleAdminMenu($currentoption = -1, $breadcrumb = "")
{
    ...
    foreach (array_keys($adminmenu) as $key) {
        $adminmenu_text .= (($currentoption == $key) ? '<li class="current">' : '<li>') . '<a href="' . $module_link
            . $adminmenu[$key]["link"] . '"><span>' . $adminmenu[$key]["title"] . '</span></a></li>';
    }
    if ($GLOBALS["xoopsModule"]->getVar("hasconfig") || $GLOBALS["xoopsModule"]->getVar("hascomments")
        || $GLOBALS["xoopsModule"]->getVar("hasnotification")
    ) {
        $adminmenu_text .= '<li><a href="' . XOOPS_URL . '/modules/system/admin.php?fct=preferences&op=showmod&mod='
            . $GLOBALS["xoopsModule"]->getVar("mid") . '"><span>' . _PREFERENCES . '</span></a></li>';
    }
    $adminmenu_text
        .= '
     </ul>
