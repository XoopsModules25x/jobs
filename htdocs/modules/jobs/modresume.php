...
if (!empty($_POST['submit'])) {
    ...
    if (!$xoopsGTicket->check(TRUE, 'token')) {
        redirect_header(XOOPS_URL . "/modules/$mydirname/resumes.php", 3, $xoopsGTicket->getErrors());
    }
    ...
    $xoopsOption['template_main'] = 'jobs_modresume.html';
    ...
    $result = $xoopsDB->query(
        "select lid, cid, name, title, status, exp, expire, private, tel, salary, typeprice, date, email, submitter, usid, town, state, valid, resume from "
    ...
    if ($xoopsUser) {
        ...
        if ($usid == $calusern) {
            ...
            $form->addElement(
                new XoopsFormText(_JOBS_RES_PCODE . " - " . _JOBS_RES_PSIZE, "private", 5, 10, $private), FALSE
            );
